...
const Footer = () => {
  const { data: settingsArray } = useContent('site_settings');
        ...
        <div className="max-w-[1400px] mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-14">
            ...
            <div className="flex items-center gap-4">
              <span className="font-body font-light text-[13px] text-muted mr-4">
                {settings.address || "Kolkata, India"}
              </span>
              {settings.instagram_url && (
                <a href={settings.instagram_url} target="_blank" rel="noreferrer" className="text-muted hover:text-foreground transition-colors hover:scale-110 inline-block" aria-label="Instagram">
                  <Instagram className="w-4 h-4" />
                </a>
              )}
              {settings.linkedin_url && (
                <a href={settings.linkedin_url} target="_blank" rel="noreferrer" className="text-muted hover:text-foreground transition-colors hover:scale-110 inline-block" aria-label="LinkedIn">
                  <Linkedin className="w-4 h-4" />
                </a>
              )}
              {settings.other_social_url && (
                <a href={settings.other_social_url} target="_blank" rel="noreferrer" className="text-muted hover:text-foreground transition-colors hover:scale-110 inline-block" aria-label="Other Social">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-14">
            ...
            <div>
              ...
              <a href={`mailto:${settings.contact_email || 'hello@thetwentyone.in'}`} className="font-body font-light text-[13px] text-muted-foreground hover:text-foreground transition-colors block mb-1">
                {settings.contact_email || 'hello@thetwentyone.in'}
              </a>
