...
const Index = () => {
  ...
  const { data: stats, loading: statsLoading } = useContent('stats', { column: 'order_index', ascending: true });
  const { data: testimonials, loading: testimonialsLoading } = useContent('testimonials', { column: 'order_index', ascending: true });
    ...
    <div ref={containerRef}>
      ...
      <section ref={reelRef} className="py-24 md:py-32 px-6 md:px-10 overflow-hidden relative">
        ...
        <div className="reel-scroll flex gap-5 overflow-x-auto pb-8 snap-x snap-mandatory" style={{ scrollbarWidth: "none" }}>
          {projectsLoading ? (
            [1, 2, 3, 4, 5].map((i) => (
              <div key={i} className="snap-start shrink-0 w-[260px] md:w-[320px]">
            ...
            (projects || []).filter((p: any) => p.featured).map((p: any, i: number) => (
              <div key={p.id} className="snap-start shrink-0 w-[260px] md:w-[320px] group relative">
                <Link to={`/work`}>
                  <div className="aspect-[3/4] overflow-hidden bg-charcoal">
                    {p.cover_image_url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-[10px] text-[#5A5A5A] uppercase tracking-widest bg-black">No Image</div>
                    )}
                  </div>
                </Link>
                <div className="absolute top-3 right-3 font-body font-light text-[11px] text-white/90 bg-black/50 px-2 py-1 tracking-wider backdrop-blur-sm">
                  {String(i + 1).padStart(2, "0")}
                </div>
                <div className="mt-3 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                  <p className="font-body font-medium text-[13px] text-foreground">{p.title}</p>
      ...
      <section className="py-24 md:py-32 px-6 md:px-10">
        <div className="max-w-[1400px] mx-auto">
          ...
          <div className="reveal-stagger grid grid-cols-1 md:grid-cols-2 gap-5">
            {servicesLoading ? (
              [1, 2, 3, 4].map(i => <Skeleton key={i} className="h-64 rounded-none" />)
            ) : (
              (services || []).map((s: any) => (
                <div key={s.id} className="reveal-child service-card group p-10 bg-accent hover:bg-charcoal transition-colors duration-500 border border-border/30 relative overflow-hidden">
                  ...
                  <span className="font-body font-light text-sm text-muted block mb-4 relative z-10">{s.number_label}</span>
                  <h3 className="font-body font-medium text-lg text-foreground mb-3 relative z-10">{s.title}</h3>
                  <p className="font-body font-light text-sm text-muted-foreground mb-6 relative z-10">{s.description}</p>
                  <Link to="/services" className="opacity-0 group-hover:opacity-100 transition-opacity font-body font-light text-sm text-foreground relative z-10">↗</Link>
                </div>
              ))
            )}
          </div>
      ...
      <section className="py-16 md:py-20 px-6 md:px-10 bg-deep-black">
        <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row items-center justify-between">
          {statsLoading ? (
            [1, 2, 3, 4].map(i => (
              <div key={i} className="flex-1 flex justify-center py-6 md:py-0 px-8">
              ...
              <div key={s.id} className="flex items-center">
                <div className="text-center py-6 md:py-0 px-8">
                  <div className="flex items-baseline justify-center">
                    <span className="stat-number font-body font-light text-[10vw] md:text-[4.5vw] text-foreground" data-target={parseInt(s.number) || 0}>
                      {s.number?.replace(/[^0-9]/g, '') || '0'}
                    </span>
                    <span className="font-body font-light text-[6vw] md:text-[2.5vw] text-foreground">
          ...
          <div className="relative">
            ...
            {testimonialsLoading || !testimonials || testimonials.length === 0 ? (
              <div className="relative z-10 mb-8 pt-8">
                <Skeleton className="h-8 md:h-12 w-full mb-3" />
                <Skeleton className="h-8 md:h-12 w-3/4 mb-16" />
                <Skeleton className="h-4 w-40 mb-2" />
                <Skeleton className="h-3 w-32" />
              </div>
            ) : (
              <>
                <blockquote className="font-display italic text-xl md:text-[2.5vw] text-foreground leading-relaxed mb-8 relative z-10 min-h-[120px]">
                  {testimonials[testimonialIdx % testimonials.length]?.quote}
                </blockquote>
                <div className="mb-8">
                  <p className="font-body font-medium text-[13px] text-foreground">{testimonials[testimonialIdx % testimonials.length]?.client_name}</p>
                  <p className="font-body font-light text-[12px] text-muted">
                    {testimonials[testimonialIdx % testimonials.length]?.client_role}
                    {testimonials[testimonialIdx % testimonials.length]?.client_company && `, ${testimonials[testimonialIdx % testimonials.length].client_company}`}
                  </p>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex gap-1.5 ml-4">
                    {testimonials.map((_: any, i: number) => (
                      <div key={i} className={`w-1.5 h-1.5 rounded-full transition-colors ${i === (testimonialIdx % testimonials.length) ? "bg-foreground" : "bg-muted/30"}`} />
                    ))}
                  </div>
                </div>
              </>
